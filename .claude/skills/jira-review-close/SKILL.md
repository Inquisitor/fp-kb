...
- **A resolved conflict is not a verified merge — build before committing.** Two traps, both hit on a real close (FP-46040 into MFT):
  - *Resolution is not necessarily uniform.* When the source commit moved lines into a new method, the identifier in scope differs between the block the target still holds and the block the source relocated. Decide each block by what is in scope at its new location; picking one side for the whole file is wrong in at least one of them.
  - *The conflict list understates the work.* Source text with no counterpart on the target applies cleanly and raises no conflict, so a reference it carries can be out of scope on the target and break the build with nothing marked. After `svn resolve`, grep the merged file for the identifiers the source introduced and confirm each resolves on the target side.

  `svn resolve` is evidence of nothing. Build the touched project — and run its tests when the merged commit carried any — before `svn commit`.
