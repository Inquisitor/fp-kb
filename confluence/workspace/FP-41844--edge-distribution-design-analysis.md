...
1. **Polynomials (pre-[FP-33182](https://fishingplanet.atlassian.net/browse/FP-33182)).** Form-specific polynomials were used to bias weight generation across the entire Unique form range. The polynomial distorted the *whole* distribution rather than just the edge, producing a characteristic double-hump artifact visible in production data. Wrong tool, wrong scope.
