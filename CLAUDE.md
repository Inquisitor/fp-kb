...
- [Observed practice is an argument, not the standard](feedback/observed_practice_vs_standard.md) — a survey answers "how is it now", one argument among several; poor discipline is not a reason to write itself into the rule, and a divergence between practice and the standard is a question to raise, not to settle silently
...
- [JIRA Story authoring format](reference/jira_story_authoring_format.md) — engineering stories: `[Kind][Subsystem]` summary tags, the Context/Problem/Goal/Scope/Out of scope/Acceptance criteria skeleton and when to use one at all, the input-vs-research boundary (no `Research` slot - findings go in comments), markup, register, typed links, fields; **read before writing a story**
...
- [Server release checklist — editing procedure](reference/release_checklist_editing.md) — confirmation gate, never delete template instructions, 🚧 is the operator's sign-off, no stale "no changes" notes, panels only for exceptions, batch edits, HTML round-trip mechanics; **read before editing any checklist page**
...
- [FP release names and server-branch mapping](reference/release_names_and_branches.md) — version-first JIRA naming, short-code synonyms (FPA / FTUE / LB-MM), release→branch map including the non-release buckets, the year umbrella and the Retail branch
