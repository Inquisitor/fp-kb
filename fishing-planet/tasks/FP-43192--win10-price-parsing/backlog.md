...
- [x] **Re-run BOTH scripts (Main + Stats) after the 2026.4 UWP release** — **bubbled to `product-local-prices/backlog.md`** on close (idempotent re-run, then confirm 0 remaining in both DBs).
...
## Deferred / later (resolved on close)
- [x] Confirm DKK parsing genuinely correct on prod (positive recent samples) — **bubbled** to `product-local-prices/backlog.md`.
- [x] Under-direction sweep: micro-prices < $0.50 conversion correctness — **bubbled** to `product-local-prices/backlog.md`.
- [x] ARS/CLP/COP USD-substitution (FP-42870 #15) + LOCAL `RegionalPriceRates` ARS check — **addressed** by the parallel crashed-currency investigation: `modules/product-local-prices/crashed-currency-region-switching.md` (mostly real crashed regional prices; only a near-zero `/1000` subset remains, tracked there as old-parser under-direction).
- [x] `MFT r16148 → NPN` merge (test mirror + SQL scripts onto the new Code branch) — **bubbled** to `product-local-prices/backlog.md`.
