...
### 2026-06-17 — /Stats/Payments aggregate fixed (third copy) + fee-migration discriminator

After the Main + Stats-fact fixes, admin `/Stats/Payments` still spiked. Traced to a THIRD copy: the `Payments` aggregate table on Stats, baked by the C# job `SalesStatUpdateJob` (AsyncProcessor -> `SqlAsyncProvider.CaptureSales`) — append-only, watermark = `MAX(Timestamp)`, per-platform rows + a `Source=NULL` total per 10-min bucket, `Amount = SUM(EquivalentPrice)` from Main. Historical buckets are never re-baked, so the inflation persisted. Watermark-rewind (`DELETE ... WHERE Timestamp >= X`) only fixes the latest spike; history needs surgical recompute.

New script `SQL/Releases/R202606-RecomputePaymentsBuckets-UWP-Stats.sql` recomputes each affected Win10 bucket from corrected Main via Stats linked-server synonyms (`MainDb*`, mirror of `CaptureSales`), overwrites the row, rebuilds the `Source=NULL` total from siblings. Counts untouched; idempotent (pure function of the window, `Transactions` never purged). Ran on XB PROD STATS: 2 same-day test buckets, then 49 historical (2023-11-24 -> 2026-05-31, regions E/S); signature re-scan shows 0 inflated E/S left.

**Detection gotcha:** a naive `recompute != stored` over-corrects, because `EquivalentPrice` was re-netted gross->net (x0.7) in Main ~Nov 2023 AFTER older buckets were baked — pre-Nov-2023 legit $99.99 buckets read `stored/recompute = 1.4286` (must NOT be touched), and ARS region-A reads `1.0`. The parser inflates x100/x1000, so the fix keys on **`stored/recompute > 3`**, which cleanly separates the bug from both. Validated: in mixed buckets the non-inflated region matches stored to the cent. 6 ARS region-A buckets left by design. Detail: [product-local-prices/log.md 2026-06-17](../../server/modules/product-local-prices/log.md).
