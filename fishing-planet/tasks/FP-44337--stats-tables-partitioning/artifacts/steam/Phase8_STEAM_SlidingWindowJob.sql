/* ============================================================================
   FP-44337  Phase 8  |  SERVER: STEAM PROD (MSSQL15.STEAMSTATS)
   ...
   A single Agent job runs it for StatsFact and MissionsFact on the 28th at 02:00 server-local
   (NY) = ~06:00 UTC daily online trough (Steam is on NY time, same as PS).

   PRE-FLIGHT (confirm before creating the job): SQL Server Agent is Running on STEAMSTATS, and
   the intended owner login is enabled (default sa - keep if enabled; else a db_owner service account).
...
/* ----------------------------------------------------------------------------
   SQL Agent job - 28th of each month at 02:00 server-local (NY) = ~06:00 UTC daily online trough.
   Two steps (one per table). Requires SQL Server Agent to be running on the instance.
   ---------------------------------------------------------------------------- */
USE msdb;
GO
