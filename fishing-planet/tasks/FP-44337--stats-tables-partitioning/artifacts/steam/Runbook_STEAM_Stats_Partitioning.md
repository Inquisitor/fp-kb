...
> adapted to Steam's numbers. Steam was NOT a disk emergency at assessment (307 GB free on 2026-07-06),
> but by 2026-08-10 `Z:` is down to **~91 GB** — urgency is now near-PS: cut over ASAP; the reclaim itself is the fix.
...
|-------|-------------------------------------------|----------------|-----------------------------------------|
| 1     | `Phase1_STEAM_LogRightsize.sql`           | PROD           | online, days before the window          |
...
> **Phase 1 on Steam is a log right-size, not a reclaim** — `Stats_log` is only ~10 GB (on PS it was 322 GB
> and the shrink was the first relief), so Phase 1 pre-GROWS it to the PS-validated ~32 GB so the Phase 3
> NCI builds / batched tail inserts never autogrow the log mid-window (log growth is always zero-initialized —
> IFI never applies to the log). The sequence is 1 (online, pre-window), 2, 3, 6, 7, 8. Phases 4/5 (SQLSTAGING
> delta) do not exist in this design (removed as redundant on PS — preservation is {pre-drop backup} U {current tail}).
...
> the DROP lossless (every `*_old` row is in it); the August tail on prod is for **continuity**, not
...
| Data file `Stats` (ROWS) | **3238 GB** at assessment (near-full); **~3347 GB and ~0.3 GB free-in-file by 2026-08-10** (per the fresh staging restore); the two fact tables are **2548 GB** of it (~79%) |
| Log file `Stats_log`     | **9.77 GB** (nothing to reclaim → Phase 1 pre-GROWS it to ~32 GB window headroom instead)                                    |
| Free on `Z:` volume      | **~307 GB** at assessment → **~91 GB by 2026-08-10** (no longer comfortable — near-PS; ~100 GB of the drop is unaccounted, audit `Z:`)        |
...
| July tail (at 2026-07-06)| `StatsFact` ~40.4 M rows (id 14,945,287,783 → 14,985,656,189); `MissionsFact` ~26.1 M (id 3,500,490,596 → 3,526,572,528); combined rate ~11 M rows/day — sizing reference for the AUGUST tail (re-measure at cutover) |
...
- **Boundaries 2026-08-01 / -09-01 / -10-01 → four partitions** (WORKING ASSUMPTION = August cutover; shift
  if the window lands in another month): an empty `< 2026-08-01` **catch-all** (own small FG), **August** (its
  OWN bounded partition — SWITCH-able when it ages into the archive), **September**, and an empty **October**
  trailing buffer (so the monthly SPLIT in Phase 8 only ever splits an empty partition). The catch-all stays
  empty (Phase 3 loads only `Timestamp >= 2026-08-01`); it is never SWITCHed out.
...
### Phase 1 — Log right-size (PROD, ONLINE, pre-window) — `Phase1_STEAM_LogRightsize.sql`

`Stats_log` is only 9.77 GB — nothing to reclaim (on PS this phase freed ~290 GB). The Steam risk is the
opposite: the Phase 3 NCI builds and batched tail inserts could outgrow a ~10 GB log and force a mid-window
autogrow — and log growth is always zero-initialized (IFI never applies to the log), which can stall the
cutover. The script **converges the log to the PS-validated ~32 GB** from either side: pre-GROW (the expected
Steam path — zero-init runs online, days before the window), shrink back if it ballooned since assessment
(guarded by `log_reuse_wait_desc`), or no-op. Idempotent. On the grow path `Z:` free drops ~22 GB
(~91 → ~69 GB at 2026-08-10 levels) — expected; budget for it.
...
3. **Create** the FGs/files (**catch-all + August + Sep + Oct**), the partition function/scheme (boundaries
   **2026-08-01 / -09-01 / -10-01** → 4 partitions; August is its OWN bounded partition), and the new
   ...
   (captured from `*_old`). Real `Z:\...\MSSQL15.STEAMSTATS\...DATA\` path; current-month `2026_08`
   `SIZE=8192MB` (bump to the measured tail size — cutover is mid-month), Sep/Oct buffers start at **1 GB**
   (`Z:` is tight), catch-all small; PAGE; IFI on. Run the column-match verification (must be 0).

### Phase 3 — Tail pre-load + START PROD (PROD, DOWNTIME) — `Phase3_STEAM_TailLoad.sql`

1. **Pre-load the August tail** old → new, with `IDENTITY_INSERT`. The insert filters **`Timestamp >= 2026-08-01`**
   (clean August partition; the id-range margin only sets the scan floor). Done IN the window, before START,
   ...
   - **`@tailFrom` = 2026-08-01** (current-month 1st) — keeps August clean. Preservation of everything older is
...
Preservation = {pre-drop FULL} U {August tail}, gap-free by Timestamp. No `bcp` / staging delta.
...
- (b) Phase 3 preload **verified complete** AND `*_old` **unchanged since Phase 3** — enforced: re-reads
  `FP44337_TailLoadControl`, re-counts the tail id-range old-vs-new (same `Timestamp >= 2026-08-01` filter),
...
> `Z:` headroom has **tightened to ~91 GB** (2026-08-10; 307 GB at assessment) — treat this near-PS: run the
...
Expected result: `Stats.mdf` ~3350 GB → ~1 TB; `Z:` free ~91 GB → ~2.4 TB.

### Phase 7 — Archive build (archive box, **DEFERRABLE**) — `Phase7_ARCHIVE_STEAM_BuildAndLoad.sql`

Build the monthly-partitioned + PAGE-compressed analyst archive and load history **< 2026-08-01** (the
dropped bulk) from the **restored STEP 0 pre-drop FULL** (its `*_old` tables are the complete source). August
and later stay on prod and arrive in the archive later via partition **SWITCH** as they age out — so align the
archive's monthly boundaries with prod's and keep empty `[Aug1,Sep1)`, `[Sep1,Oct1)`, `[Oct1,…)` landing
...
partition. Pre-flight: Agent running on STEAMSTATS + owner login enabled (default `sa`). After an August cutover
the first real action is **28 September** (adds November). New month files start at **1 GB**
(`@InitSizeMB` default; `Z:` is tight) and autogrow in 4 GB IFI-instant steps — safe even if the Phase 6
shrink is delayed; pass a bigger `@InitSizeMB` post-shrink if pre-sized files are preferred.
...
> Note: P1 (`< 2026-08-01`) is an unbounded left catch-all and stays EMPTY. Aged-out-month archiving
> `SWITCH OUT`s from the bounded month partitions (August onward), never P1. August is its own bounded partition
...
| Pre-load August tail, both tables (~66 M rows, batched)    | 12-18 min         |
...
the tail the day of cutover to size the August file and the load time.)

> Pre-flight (before the window): re-measure the August tail the day of cutover; confirm IFI enabled and
> `log_reuse_wait_desc = NOTHING`; log pre-grown to ~32 GB (Phase 1); confirm the tempdb location + cap.
> `@tailFrom` = current-month 1st.

---

## Rollback

- **Before DROP (Phase 6):** fully reversible. Stop prod, drop the new tables (only the August tail),
...
- **After DROP:** prod history is gone from prod but preserved as the pre-drop full backup (and, once built,
  the archive); the August+ tail is in the new prod partition. Recovery is restore/backfill — not an in-window
...
| Risk                                                        | Mitigation                                                                                                                                    |
|-------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------|
| New files autogrow eats disk before shrink frees space      | `Z:` tight (~91 GB @ 2026-08-10): Sep/Oct buffers start at 1 GB, pre-size only the current month; cut over ASAP; step the Phase 6 shrink; monitor `xp_fixeddrives` |
| Mid-window log autogrow (zero-init) stalls the NCI build    | Phase 1 pre-grows `Stats_log` to the PS-validated ~32 GB days before the window (log growth is never IFI-instant)                             |
...
| Monthly SPLIT moves data (non-empty catch-all)              | 2-month empty buffer (Sep+Oct) + catch-up loop; proc **THROWs** if the split target isn't empty; alert on job failure                          |
