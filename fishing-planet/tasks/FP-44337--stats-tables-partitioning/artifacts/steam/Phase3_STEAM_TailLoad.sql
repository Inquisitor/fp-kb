/* ============================================================================
   ...
   reads dbo.StatsFact WHERE EntityId > @lastCursor, and its cursor typically lags
   real time by ~1 hour. After the rename those unprocessed recent rows live only
   in StatsFact_old; the job reads the NEW table and would SKIP them, leaving a gap
   in fishing-rate stats. Loading the tail (with original EntityId via IDENTITY_INSERT;
   old ids < the new IDENTITY base, so no collision) restores continuity. Same reasoning
...
BEGIN
    ...
    -- Re-apply the identity cushion. The TRUNCATE above RESET the identity counter to the column's
    -- ORIGINAL seed (TRUNCATE discards the Phase 2 RESEED); the IDENTITY_INSERT load then pulled the
    -- counter up to MaxOldId only - so without this, live rows start at MaxOldId+1 (collision-free,
    -- since all tail ids are <= MaxOldId, but WITHOUT the intended +1M cushion). Observed live on the
    -- Steam cutover 2026-08-10 (harmless there; gate ranges are capped at MaxOldId and stay correct).
...
-- (Replicated from PS: supports per-UserId time-range lookups; partition-aligned so it never
--  blocks a future partition SWITCH.)
