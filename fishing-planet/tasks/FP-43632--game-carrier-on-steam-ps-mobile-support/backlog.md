...
- [ ] **Orphaned peer objects on the Game application over TCP**. Peers whose connection dropped before authentication are never released — the oldest has been alive 220 days on Nintendo — while the same TCP on a Master node, and the same Game application over WSS and QUIC, stay clean. Counters show the sockets are already closed, so the objects outlive them. Put the matrix to GC dev together with the earlier failure to stop the server cleanly, where lws was named as the blocker: both are the same question of when a GameCarrier transport releases a connection. Evidence: [artifacts/orphaned-peers-2026-09-17.md](artifacts/orphaned-peers-2026-09-17.md)
- [ ] **After the release: check the sockets-versus-objects reading on Nintendo**. Counters arrive there with 2026.5.1. Compare `Tcp.Network.Client.Connections.Current` against the `Photon + TCP` peer count in the trace, which sits at 189 with 121 of them orphaned — a hundredfold gap would settle the diagnosis, a matching figure would overturn it.
