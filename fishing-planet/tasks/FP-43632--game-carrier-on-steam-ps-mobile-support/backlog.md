...
- [ ] File new JIRA tickets adjacent to FP-43632; add links to them in this journal.
  ...
  - [ ] Track 3 — Local dev environment
- [x] Migration runbook home: settled as the release-checklist section. The rollout procedure lives in [2026.5.1 - GameCarrier Migration Server Release checklist](https://fishingplanet.atlassian.net/wiki/spaces/FP/pages/5960925185), one pass per platform farm, alongside the other server release checklists.
- [x] **Confluence page: server transport ports / vhosts distribution** — published 2026-05-10 as [Server Transport Ports](https://fishingplanet.atlassian.net/wiki/spaces/FP/pages/5579014145) under Infrastructure, and kept current since: the endpoint table, the canonical `vhosts[]` ordering, the Xbox exception and where configs live.
- [ ] **Chat port cleanup (tech-debt)**. Today the Chat app rebases between `4520` (Chat-only node, where Master S2S is absent) and `4521` (AllInOne / Master node, where 4520 is taken by Master S2S) depending on node role. When the upcoming common-template-plus-platform-overrides config rework lands, fix Chat at `4521` universally to remove the rebase mechanic. No port collision then — Master S2S keeps 4520, Chat stays on 4521 everywhere, Club stays on 4522. Relates: FP-46180 (the ticket this parking lot became; out of scope for 2026.5.1 because the Chat port can only change for all nodes at once, so it ships with 2026.6 Australia)
...
- [ ] **GameCarrier assembly-binding injection — product-level question**. `GC.Runtime` rewrites the hosted application's binding redirects when it creates the app-domain (`photon-server-integration/GC.Runtime/AssemblyBinding.cs`, added 2026-07-08 in `024f9fe7`; the SVN counterpart r16278 removed the per-config copies that became redundant). Three concerns — none blocking this migration, all relevant if GameCarrier is ever licensed out: (1) the host **replaces** the guest's `<assemblyBinding>` section wholesale instead of filling gaps, so an application that deliberately pins a dependency version loses that pin silently; (2) the section is located by literal `IndexOf` on the opening tag, so any variation in attribute order, quoting or line breaks makes injection a silent no-op — and the failure surfaces far from its cause; (3) the effective configuration is a generated `*.binding.config` beside the deployed one, so the file an engineer inspects is not the file the app-domain loads. Possible directions: merge rather than replace, make injection opt-in, or resolve versions through the `AssemblyResolve` hook the adapter already installs (`bridge.AddAssemblyResolve` in `Adapter.cs`). Investigate, then escalate to GC dev as appropriate.
- [ ] **TCP connections on the Game application are never released**. A client that establishes a TCP connection and then sends nothing keeps it `Established` indefinitely: the GameCarrier TCP transport never sets a connection timer - `tcp.api.calls.connectionsettimer.total` is zero across every connection the PlayStation nodes have accepted - and neither an application timeout nor TCP keepalive covers the gap. About one per day per node, plus a batch when a node enters rotation; the oldest on Nintendo has stood for 220 days. QUIC is unaffected, since msquic applies its own idle timeout. Photon on the same farm keeps three such sockets over 48 days against GameCarrier's eight over four, so this is a difference of degree between the transports rather than a defect unique to one. Put it to GC dev together with the earlier failure to stop the server cleanly, where lws was named as the blocker: both are the same question of when a transport releases a connection. Evidence: [artifacts/orphaned-peers.md](artifacts/orphaned-peers.md)
