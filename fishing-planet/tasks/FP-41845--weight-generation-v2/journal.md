...
## Status
Phase 2a complete (r15918–r15937, commits 1–7). Phase 2c: r15963 (form-specific flags) + r15987 (scope editor UI, enum cleanup, MVC refactoring), both merged to MFT. Next: Phase 2c follow-up — update KB and Confluence docs. Phase 3 (documentation) complete — FP-41844 closed. Phase 2b (crossover visualization) deferred.
...
- 2026-04-06: Commit 8 (r15963) — Form-specific edge distribution scope. New form-specific flags in `EdgeDistributionScope` (Young/Common/Trophy/Unique × Upper/Lower), `YoungAndHeaviest` preset (GD-requested: upper on heaviest + lower on Young only). `GetEdgeFlags()` form-specific OR layer, WebAdmin dropdown with scope normalization for Kendo compatibility. Tests for form-specific matching, combo preset, missing-form species. 60 total green. Merged → MFT r15964.
- 2026-04-08: Commit 9 (r15987) — Scope editor UI, enum cleanup, MVC refactoring. Scope editor modal with Role/Form checkbox matrix and SVG preview (bezier curves, 4-row species table). Inline SVG scope preview replacing Kendo dropdown. Removed all enum presets — `ToString()` always returns individual flag names. MVC: `EdgeDistributionSettingsModel` for DB read/write via `DalFactory`; controller no longer touches cache. `ISysProvider.GetGlobalVariable()`/`SetGlobalVariable()` — new DAL methods. Swapped Common/Unique colors. 60 tests green. Merged → MFT r15988.
