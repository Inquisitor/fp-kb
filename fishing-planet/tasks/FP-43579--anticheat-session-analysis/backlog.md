...
- [x] SVN commit — done 2026-05-05 (LBM r16063) covering Mongo + DAL + WebAdmin (controller / models / view / Components/AntiCheatTool / Scripts/vue/anti-cheat / RouteConfig + 2 modified shared views + csproj). Merged into MFT (Code) r16064.
...
- [~] Run existing `Sql.MsSql.Tests` and `WebAdmin.Tests` after signature change — **dropped on close**: code review trusted the variant-B change (optional params, default no-op for the single existing caller; no Moq mocks affected). No regression observed since 2026-05-05 deploy. Revisit if `IAnalyticsProvider` touched again.
...
- [x] **DOC-002** — DONE 2026-05-12. Created `<kb>/fishing-planet/server/modules/web-admin/` as umbrella stub: `_card.md` (status `stub`, umbrella style, conventions/gotchas firm, controller inventory TBD pending FP-43424 Pass 2/3), `log.md` (milestone + 5 findings cross-linked to KB feedback/reference), [`embedded-vue-pattern.md`](../../server/modules/web-admin/embedded-vue-pattern.md) deep-dive (full pattern: stack rationale, embed flow, routing, build pipeline, csproj registrations, state mgmt, Kendo bridge, layout constraints, checklist, TargetedAdsPlanningTool contrast table). Registered in `<kb>/fishing-planet/server/_index.md` under new section «Operations & Admin».
