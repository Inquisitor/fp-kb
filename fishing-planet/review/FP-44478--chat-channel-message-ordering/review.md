...
- Post-review damage recon spun off as its own task **FP-44670** (the code review itself is resolved/Approve). Signal: re-deliveries are traceable in `clubLog` as byte-identical `Received bait #…`/`Received ClubToken …` lines repeated for one `(UserId, requestId)`. Mechanism/window/scope notes and runnable Mongo aggregations live in the task folder: [`damage-recon.md`](../../tasks/FP-44670--club-donation-redelivery-damage/damage-recon.md) + [`damage-recon-queries.js`](../../tasks/FP-44670--club-donation-redelivery-damage/damage-recon-queries.js). Queries syntax-validated on local Mongo 4.4.13; prod runs pending.
