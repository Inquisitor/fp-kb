...
- [ ] Eliminate cross-platform schema divergences across the per-platform Stats databases — same-named tables carry different columns per platform (e.g. `Users.ExternalId` absent on Mobile/Nintendo; `Profiles.DuelRating`/`EventRating` absent on PS; the tournament result/participant tables diverge along Steam/Mob/Nx vs PS/Xbox lines), and archive/temp/backup leftovers (`StatsFact20xx`, `*_old`, `*_bak`, `UsersTemp29`, `FishFactTemp`, Steam-only `R_*` reporting views, and a `VW_TopReferralCheaters` broken by a missing linked server) linger unevenly. Drift complicates cross-platform analytics and any code assuming a uniform Stats schema. Full per-column matrix: `<kb>/reference/stats_platform_schema_divergence.md`. (Surfaced while provisioning the analytics read-only login, `<kb>/reference/stats_analyst_ro_access.md`.)
