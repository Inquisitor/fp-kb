...
- The mission position operation (`MissionOperationCode.PlayerPositionAndRotation`: 24 raw bytes of position and rotation plus an optional aim point, handled in `GameClientPeer_Missions`) is removable — verified NPN20260602, 2026-09-09. (1) The aim point it writes has a fallback: `RodInGameMissionsContext.TargetPosition` returns the fight wire's throw target (`tT`, read in `GameProcessor`) when the operation never set it, so `TargetInBoxCondition.Check` evaluates the throw target and cast-into-area missions still complete on the Throw transition (`CastCondition.Check`); only the pre-cast "aiming into the area" hint disappears. (2) The rotation it carries is read by nobody. (3) Player position for `PlayerDistanceCondition` and the in-box conditions also arrives via the walk/boat opcodes at roughly a 2 s cadence, so zone conditions will react with that lag — the one thing to check by hand before removal. Rollout order: server stops reading and answers OK first, client stops sending later, otherwise old clients get errors. The client code has carried a "must be disabled soon" note since 2022. Origin: FP-45122 scope discussion — the client side proposed switching this path off inside Fish Fight Protocol v2; the server lead's decision is a separate v1 cleanup owned by the server side. Relates: FP-45122 (out of v2 scope), TPMv3 (one position path before the avatar rewrite; see [tpm](../tpm/_card.md)).
