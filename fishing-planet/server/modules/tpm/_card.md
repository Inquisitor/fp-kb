...
> Other players' avatars: the client sends a binary frame about itself every 200 ms as an unreliable Photon event; the server relays it to the room without reading the bytes. Cosmetic channel — nothing in gameplay depends on it. Known-wrong content model (first-person rod bend replayed on a third-person rig) and a rewrite epic TPMv3 exist. Explicitly OUT of the FP-45122 Fish Fight Protocol v2 scope, sequenced after it.
...
## Key Facts
- Wire: `OpRaiseEvent(EventCode.CharacterEvent)`, unreliable, one frame per message, Hashtable {event type, userId, byte[] frame}, ~55 bytes without lists and boat. Frame: position, rotation, boat, shown fish and items, rods and rod pods, 22 animation parameters, timestamped actions
