...
- FP-43631 (completed 2026-05-11): PCR-drop abuse detection — discriminator (`IsStarted = 0`), threshold heuristic (`NoShowSharePct ≥ 30`), and per-period surgical leaderboard ban via `CompetitiveRatingsCurrent.IsBanned`. 29 banned pre-finalization across STEAM/PS/XB; 107 in wider sweep routed to Support. **Correction (2026-08-31)**: the earlier note here claimed `BracketId` maps to `1=NOOBS / 2=MIDDLES / 3=TOPS`. It does not — `TournamentParticipants.BracketId` is the bucket after `BalanceBuckets` has filled and merged, so a player can sit in a bucket well away from his own band (a rating of 20562 observed in bucket 1). Agreement with the standard bands is ~92% and the misses cluster in thin or oddly-configured competitions. Use `CompetitionRatingAtStart` (FP-43816) when the rating band is what you actually mean
